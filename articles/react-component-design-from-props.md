...
もちろん、optional な props 自体が悪いわけではありません。
例えば `disabled?: boolean` のように、その値があるかどうか自体が UI の自然な状態を表しているケースもあります。

問題になりやすいのは、複数の props の組み合わせによって、暗黙的にコンポーネントの状態を表現しているケースです。
その場合、props の関係性や取りうる状態が型から読み取りづらくなり、不正な状態も表現できてしまいます。
...
これは見方を変えると、`UserList` が取りうる UI の状態空間を props の型として定義しているとも言えます。
`loading` / `error` / `success` という3つの状態だけを許可し、それ以外の組み合わせを型によって表現できないようにしている、ということです。
...
結果として、コンポーネント使用者はどの状態を渡せば良いかがわかりやすくなり、コンポーネント実装者は存在しない状態を考慮しなくてよくなります。利用側・実装側の双方にとって扱いやすい設計になります。

言い換えると、props は単なる値の集合ではなく、UI の状態を宣言するためのものとして捉え直すことができます。

ここでいう「状態」は、loading / error / success のような画面状態だけを指しているわけではありません。
コンポーネントがどのように振る舞うのか、どんな責務を持つのか、といった設計上の意図も含めて考えることができます。
...
もちろん、リンクとして遷移しつつクリック時に何か処理をしたいケースもあります。
そのため、`href` と `onClick` を同時に受け取れる設計が常に間違っている、というわけではありません。

ここで考えたいのは、このコンポーネントがどのような責務や振る舞いを持つのかが props から読み取れるかどうかです。
例えばこの `LinkButton` が、リンクとして振る舞う場合とボタンとして振る舞う場合を排他的に扱いたいコンポーネントなのであれば、次のように props を定義できます。
...
props は単なる引数ではなく、コンポーネント利用者に公開される API でもあります。
だからこそ、props を単なる値の集合として扱うのではなく、「この props はコンポーネントの状態や責務を正しく表現できているだろうか？」という視点で見てみると、React コンポーネント設計についてまた違った見え方ができるかもしれません。
